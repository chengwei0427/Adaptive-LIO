...
namespace zjloc
{
    ...
    public:
        ...
        /// 后向递推
        bool Back_Predict(const IMU &imu);
        /// 前向递推
        bool Predict_Cont(const IMU &imu);
        ...
        NavStateT GetBackPredictState() const { return NavStateT(current_time_unc, R_unc, p_unc, v_unc, bg_, ba_); }
        NavStateT GetPredictContState() const { return NavStateT(current_time_cont, R_cont, p_cont, v_cont, bg_, ba_); }

        /// 获取SE3 状态
        SE3 GetNominalSE3() const { return SE3(R_, p_); }

        SE3 GetBackPredictSE3() const { return SE3(R_unc, p_unc); }
        SE3 GetPredictContSE3() const { return SE3(R_cont, p_cont); }

        /// 获取速度
        VecT GetNominalVel() const { return v_; }

        VecT GetBackPredictVel() const { return v_unc; }
        VecT GetPredictContVel() const { return v_cont; }
        ...
        void SetX(const SE3 &pose, const Vec3d &v)
        {
            p_ = pose.translation();
            v_ = v;
            R_ = pose.so3();
        ...
        void prepareBack()
        {
            current_time_unc = current_time_;
            R_unc = R_;
            p_unc = p_;
            v_unc = v_;
        }
        ...
        void prepareCont()
        {
            current_time_cont = current_time_;
            R_cont = R_;
            p_cont = p_;
            v_cont = v_;
        }

        void SetInitialTime(const double t) { current_time_ = t; }
    ...
    private:
        ...
        void UpdateAndReset()
        {
            ...
            // std::cout << "before:" << v_.transpose() << std::endl;
        ...
        //
        double current_time_unc = 0.0;
        VecT p_unc = VecT::Zero();
        VecT v_unc = VecT::Zero();
        SO3 R_unc;

        //
        double current_time_cont = 0.0;
        VecT p_cont = VecT::Zero();
        VecT v_cont = VecT::Zero();
        SO3 R_cont;
    ...
    template <typename S>
    bool ESKF<S>::Back_Predict(const IMU &imu)
    {
        ...
        v_unc = v_unc + (R_unc * (imu.acce_ - ba_) + g_) * dt;
        p_unc = p_unc + v_unc * dt -
    ...
    bool ESKF<S>::Predict_Cont(const IMU &imu)
    {
        ...
        // nominal state 递推
        VecT new_p = p_cont + v_cont * dt + 0.5 * (R_cont * (imu.acce_ - ba_)) * dt * dt + 0.5 * g_ * dt * dt;
        VecT new_v = v_cont + R_cont * (imu.acce_ - ba_) * dt + g_ * dt;
        SO3 new_R = R_cont * SO3::exp((imu.gyro_ - bg_) * dt);
